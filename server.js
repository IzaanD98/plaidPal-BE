...
app.post("/api/signup", async (req, res) => {
  ...
  try {
    // if (true) {
      ...
      const results = await User.find({ googleId: verificationResponse?.payload.sub });
      if(results.length > 0){
        return res.status(409).send({message: "PlaidPal Error: User already exists"});
      }
      ...
      res.status(201).send({
        ...
        .catch((error) => {
          ...
          return res.status(500).send({ message: "An error occurred. Registration failed. 1" });
        });
    }
  } catch (error) {
    console.log('plaidPal error 2nd catch:');
    console.log(error);
    return res.status(500).send({ message: "An error occurred. Registration failed. 2" });
  }
});
